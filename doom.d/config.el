...
(setq treemacs-filewatch-mode t)            ; обновлять при изменениях на диске
(setq treemacs-git-mode 'deferred)          ; git-статус (не тормозит)
...
(after! lsp-mode
  (setq lsp-pylsp-plugins-pycodestyle-indent-size 2))

;; LATEX config ======================
(after! org
  ;; Увеличиваем масштаб формул в 1.5 раза
  (plist-put org-format-latex-options :scale 1.5)

  ;; Настраиваем разрешение (DPI)
  (plist-put org-format-latex-options :background "Transparent")

  ;; Автоматически показываем все формулы при открытии org-файла
  (add-hook 'org-mode-hook
            (lambda ()
              (org-latex-preview '(16))))
)

;; Отключаем org-fragtog - он мешает постоянному отображению
